...
__version__ = "%s.4.11"  % sys.version.split('.', 1)[0]
...
class Transport:
    ...
    def send_request(self, host, handler, request_body, debug):
        ...
        if self.accept_gzip_encoding and gzip:
            connection.putrequest("POST", handler, skip_host=self._host_name, skip_accept_encoding=True)
            headers.append(("Accept-Encoding", "gzip"))
        else:
            connection.putrequest("POST", handler, skip_host=self._host_name)
...
def sse(url, last_event_id=None, api_key=None, host_name=None):
    """
    g = sse(url)
    ...
    while True:
        ...
        try:
            f = urllib.request.urlopen(urllib.request.Request(url, data=None, headers=headers), timeout=5+random.random())
            event, data, event_id = "message", None, ""
            #while True:
            #    v = f.fp._sock.fp.readline()
            for v in f:
                v = v.decode()
                if '' == v:
                    break
        ...
        time.sleep(retry / 1000.0 + random.random())
