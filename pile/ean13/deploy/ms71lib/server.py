...
The Doc* classes can be used to create JSON-RPC servers that
...
from ms71lib.client import Fault, dumps, loads, gzip_encode, gzip_decode
...
class SimpleJSONRPCDispatcher:
    ...
    def _marshaled_dispatch(self, data, dispatch_method = None, path = None):
        ...
        #print(444, type(data))
        try:
            params, kwargs, method = loads(data, use_builtin_types=self.use_builtin_types)
        ...
        return response  # .encode(self.encoding, 'jsoncharrefreplace')

    def system_listMethods(self, method_name=None):
        """system.listMethods() => ['add', 'subtract', 'multiple']

        Returns a list of the methods supported by the server."""

        if self.instance and method_name:
            method = resolve_dotted_attribute(
                self.instance,
                method_name,
                self.allow_dotted_names
            )
            methods = set(list_public_methods(method))
...
class SimpleJSONRPCRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if not path:
            path = '/'
        if self.rpc_paths:
            if not (path in self.rpc_paths) or not method:
                self.report_404()
                return
        ...
        try:
            ...
            if len(pq) > 1:
                for kv in pq.pop(1).split('&'):
                    kv = kv.split('=', 1)
                    if len(kv) > 1:
                        kwargs[urllib.parse.unquote(kv[0]).strip()] = urllib.parse.unquote(kv[1]).strip()
                    else:
                        params.append(urllib.parse.unquote(kv[0]).strip())
            #print("method:", method)
            #print(params)
            #print(kwargs)
            data = dumps(params, kwargs, method, methodresponse=None, encoding=None, allow_none=True)
            #print self.server
            #print data
    ...
    def _marshaled_dispatch(self, data, dispatch_method = None, path = None):
        try:
            ...
            response = dumps(
                Fault(1, "%s:%s" % (exc_type, exc_value)),
                encoding=self.encoding, allow_none=self.allow_none)
            #response = response.encode(self.encoding, 'jsoncharrefreplace')
        return response
...
class SCGIJSONRPCRequestHandler(SimpleJSONRPCDispatcher):
    """Simple handler for JSON-RPC data passed through SCGI."""

    def __init__(self, allow_none=True, encoding=None, use_builtin_types=False):
        SimpleJSONRPCDispatcher.__init__(self, allow_none, encoding, use_builtin_types)

    def handle_request(self, env, request_text=None):
        """Handle a single JSON-RPC request passed through a SCGI post method.
        ...
        if request_text is None and env['HTTP_METHOD'] == 'GET':
            uri = env["URI"]
            if uri and uri[-1] == '/':
                method = uri[:-1].split('/')[-1]
            else:
                method = uri.split('/')[-1]
            request_text = dumps(env['HTTP_PARAMS'], env['HTTP_KWARGS'], method, methodresponse=None, encoding=None, allow_none=1)
            ...
            yield '%d %s' % (code, message)
            yield [
                ('Content-Type', http.server.DEFAULT_ERROR_CONTENT_TYPE),
                ('Content-Length', str(len(response)),
            ]
            yield response
            """
            ...
            if request_text is None and length:
                request_text = env['scgi.rfile'].read(length)
                if "gzip" == env.get("HTTP_CONTENT_ENCODING", env.get("CONTENT_ENCODING")):
...
# -----------------------------------------------------------------------------
# Self documenting JSON-RPC Server.
...
class ServerHTMLDoc(pydoc.HTMLDoc):
    ...
    def docserver(self, server_name, package_documentation, methods):
        """Produce HTML documentation for an JSON-RPC server."""
...
class JSONRPCDocGenerator:
    """Generates documentation for an JSON-RPC server.
    ...
    def __init__(self):
        ...
        self.server_name = 'JSON-RPC Server Documentation'
        self.server_documentation = \
            "This server exports the following methods through the JSON-RPC "\
            "protocol."
        self.server_title = 'JSON-RPC Server Documentation'
...
class DocJSONRPCRequestHandler(SimpleJSONRPCRequestHandler):
...
class DocJSONRPCServer(  SimpleJSONRPCServer,
                        JSONRPCDocGenerator):
    """JSON-RPC and HTML documentation server.

    Adds the ability to serve server documentation to the capabilities
    of SimpleJSONRPCServer.
    """

    def __init__(self, addr, requestHandler=DocJSONRPCRequestHandler,
                 logRequests=True, allow_none=True, encoding=None,
                 bind_and_activate=True, use_builtin_types=False):
        SimpleJSONRPCServer.__init__(self, addr, requestHandler, logRequests,
                                    allow_none, encoding, bind_and_activate,
                                    use_builtin_types)
        JSONRPCDocGenerator.__init__(self)

class DocCGIJSONRPCRequestHandler(   CGIJSONRPCRequestHandler,
                                    JSONRPCDocGenerator):
    """Handler for JSON-RPC data and documentation requests passed through
    CGI"""
    ...
    def __init__(self):
        CGIJSONRPCRequestHandler.__init__(self)
        JSONRPCDocGenerator.__init__(self)
...
if __name__ == '__main__':
    ...
    server.register_function(pow)
    server.register_function(lambda x,y: float(x)+float(y), 'add')
    server.register_instance(ExampleService(), allow_dotted_names=True)
    server.register_multicall_functions()
    print('Serving JSON-RPC on localhost port 8000')
